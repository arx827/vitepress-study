...
## 布局模板 (Layouts)
  - `Layout` 放在 `layouts/` 目錄
  - 在 `app.vue` 中，添加 `<NuxtLayout />` 元件來表示使用布局模板，也可以通過 `name` 設定不同的模板名稱。

## 建立一個預設的布局模板
  - 預設 Layout：
    - `layouts/default.vue`
  - 所有頁面會自動套用 `default layout`
  - 如果在頁面元件中未特別指定要使用哪個模板或 `<NuxtLayout />` 沒有設定 `name` 屬性，那麼都將會使用 `default` 作為預設的布局。
